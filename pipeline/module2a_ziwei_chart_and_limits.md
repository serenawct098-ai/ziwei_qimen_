...
> 治理提醒：本節（9.5重忌判定）與A01–A19同屬模組2A輸出範圍，`heavy_ji` 屬於規格書11.3節定義的 `ziwei_contextual_risk_inputs`，只作盤面推演、風險條件、時間窗口及行動修正之用，不得直接輸入紫微五級評級（詳見effective_rules.md「紫微脈絡資料」條目及module3a_ziwei_inference.md）。
