...
1. 依陰陽遁及局數直接查 `data/qimen/earth_plate_9_ju_table.json` 完整九局地盤表，不得由摘要口訣即席拼接或推導九宮序列。
...
> 本步驟之六局×六旬首完整枚舉查表由 `data/qimen/xunshou_zhifu_table.json` 提供；該表現階段為骨架（`extraction_status: draft`，內容標記 `_todo_note` 待補），B03邏輯先依本節文字規則執行，待查表核准後改為查表模式。
...
> `nine_palace_rotation_table.json` 現階段為骨架，B04依本節文字規則（值符隨時干飄轉，其餘八星依旬首原始順序同步平移）執行，待查表核准後改為查表模式。
...
> `zhishi_door_table.json` 現階段為骨架，B05依本節文字規則執行，待查表核准後改為查表模式。
...
> `void_horse_table.json` 現階段為骨架，規格書本節僅述規則文字，未提供完整六十甲子旬空及驛馬枚舉對照表，詳細缺口說明見該JSON檔 `_todo_note` 欄位。
